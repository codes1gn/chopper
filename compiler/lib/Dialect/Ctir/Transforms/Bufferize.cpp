...
static SmallVector<Value, 6> bypassResultShapes(Operation &op) {
  ...
  if (auto broadcastTo = dyn_cast<ctir::BroadcastToOp>(op)) {
    return {broadcastTo.shape()};
  }

  if (auto splatted = dyn_cast<ctir::SplattedOp>(op)) {
    return {splatted.shape()};
  }

  if (auto pad = dyn_cast<ctir::PadOp>(op)) {
...
namespace {
// TODO: Lower to a "buffer version" of ctir::BroadcastTo instead of directly to
// loops.
class LowerBroadcastToToLoopsPattern
    : public OpConversionPattern<ctir::BroadcastToOp> {
public:
  using OpConversionPattern::OpConversionPattern;
  LogicalResult
  matchAndRewrite(ctir::BroadcastToOp op, ArrayRef<Value> operands,
...
namespace {
class BufferizeSplattedOp : public OpConversionPattern<ctir::SplattedOp> {
public:
  using OpConversionPattern::OpConversionPattern;
  LogicalResult
  matchAndRewrite(ctir::SplattedOp op, ArrayRef<Value> operands,
                  ConversionPatternRewriter &rewriter) const override {
...
namespace {
class BufferizePadOp : public OpConversionPattern<ctir::PadOp> {
public:
  using OpConversionPattern::OpConversionPattern;
  LogicalResult
  matchAndRewrite(ctir::PadOp op, ArrayRef<Value> operands,
                  ConversionPatternRewriter &rewriter) const override {
    auto resultsOrFailure = allocateResults(op, rewriter, op.getLoc());
    if (failed(resultsOrFailure))
      return failure();
...
namespace {
class CtirBufferizePass : public CtirBufferizeBase<CtirBufferizePass> {
  ...
  void runOnOperation() override {
    ...
    target.addIllegalOp<ctir::BroadcastToOp>();
    patterns.add<BufferizeSplattedOp>(typeConverter, context);
    target.addIllegalOp<ctir::SplattedOp>();
    patterns.add<BufferizePadOp>(typeConverter, context);
    target.addIllegalOp<ctir::PadOp>();
