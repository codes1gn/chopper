...
void JITModule::buildBackendCompilationPipeline(PassManager &pm,
                                                bool optimize) {
  CHOPPER::RefBackendLoweringPipelineOptions options;
  options.optimize = optimize;
  CHOPPER::createAtirRefBackendLoweringPipeline(pm, options);
}
...
JITModule::fromCompiledModule(mlir::ModuleOp module,
  ...
  auto expectedAddress = ret->engine->lookup("__chopper_module_descriptor");
  if (!expectedAddress)
