...
impl Functor {
    ...
    pub fn apply(
        ...
        lhs_buffer_functor: DataView<concrete_backend::Backend>,
        rhs_buffer_functor: DataView<concrete_backend::Backend>,
    ) -> DataView<concrete_backend::Backend> {
        let mut res_buffer_functor = DataView::<concrete_backend::Backend>::new(
        ...
        // TODO wrap this part relate to the opcode
        let new_constant: u32 = 1;
        // TIPS
        // system stores data in Little Endian order,
        // BE 1 will become 16777216 in LE format <u32>
        //let spec_bytes = new_constant.to_be_bytes();
        let spec_bytes = new_constant.to_le_bytes();
        println!("{:?}", spec_bytes);
        let spec_data: Cow<[u8]> = Cow::Borrowed(&spec_bytes);
