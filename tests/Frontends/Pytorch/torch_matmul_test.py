...
def do_test(shape1, shape2):
    ...
    lhs_input_ref = torch.empty(shape1, dtype=torch.float32).uniform_().clone().detach().requires_grad_(True)
    rhs_input_ref = torch.empty(shape2, dtype=torch.float32).uniform_().clone().detach().requires_grad_(True)
