...
//===----------------------------------------------------------------------===//
//
// This is the public-facing interface for interacting with the chopper
// runtime.
//
...
//
// chopper/RefBackend/Runtime/Support.h provides some minimal LLVM-like support
// code to keep the API familiar.
//
//===----------------------------------------------------------------------===//

#ifndef CHOPPER_RUNTIME_USERAPI_H
#define CHOPPER_RUNTIME_USERAPI_H

#include "RefBackend/Runtime/Support.h"
#include <array>
#include <atomic>
#include <cstdlib>
...
#define DEFINE_IS_REF(x)                                                       \
  if (is##x()) {                                                               \
    ...
    CHOPPER_FORALL_REF_TAGS(DEFINE_IS_REF)
#undef DEFINE_IS_REF
    return false;
  }
...
#define DEFINE_CASE(x)                                                         \
  case Tag::x:                                                                 \
    return #x;
      CHOPPER_FORALL_TAGS(DEFINE_CASE)
#undef DEFINE_CASE
    }
...
#define DEFINE_INCREF(x)                                                       \
  if (is##x()) {                                                               \
    ...
    return;                                                                    \
      ...
      CHOPPER_FORALL_REF_TAGS(DEFINE_INCREF)
#undef DEFINE_INCREF
...
#define DEFINE_DECREF(x)                                                       \
  if (is##x()) {                                                               \
    ...
    return;                                                                    \
      ...
      CHOPPER_FORALL_REF_TAGS(DEFINE_DECREF)
#undef DEFINE_DECREF
      assert(false && "Unsupported RtValue type");
    }
  }
...
#define DEFINE_TAG(x) x,
    CHOPPER_FORALL_TAGS(DEFINE_TAG)
#undef DEFINE_TAG
  };
...
#endif // CHOPPER_RUNTIME_USERAPI_H
